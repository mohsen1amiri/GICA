# baseline_lingifa.py  (test-time-scaling adaptation)
import math
import time
...
class LinGIFA:
    """LinGIFA baseline for the test-time-scaling pipeline.

    Concrete instance of the GIFA (Gap-Index Focused Allocation) framework of
    Réda et al. (2021), Table 1. In the GICA paper it is one of the linear-bandit
    baselines: each candidate reasoning path is an arm with feature ``g(pi)`` (the
    length-normalized mean of its step features), and pulling an arm yields a noisy
    verifier observation used to update a shared least-squares estimate of theta*. (This
    TTS variant is identical to the synthetic implementation; it is invoked by the TTS
    drivers through the PRM oracle.)
