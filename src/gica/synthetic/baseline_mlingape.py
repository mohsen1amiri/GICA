...
class m_LinGapE:
    """m-LinGapE baseline for fixed-confidence top-m identification in linear bandits.

    This is the top-m form of LinGapE (Xu et al., 2018), as tabulated in Table 1 of the
    LinGIFA paper (Réda et al., 2021). In the GICA paper it serves as a linear-bandit
    baseline: each candidate reasoning path is an arm with feature ``g(pi)`` (the
    length-normalized mean of its step features), and pulling an arm yields a noisy
    observation of its utility used to update a shared least-squares estimate of theta*.
    ...
    def _C(self) -> float:
        ...
        ``C_{t,delta} = sqrt( 2 log(1/delta) + d log(1 + (t+1) L^2 / (lambda^2 d)) )
                        + (sqrt(lambda) / sigma) * S``.
        """
